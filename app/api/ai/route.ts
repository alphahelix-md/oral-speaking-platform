...
export async function POST(request: Request) {
  try {
    ...
    if (input.action === 'next') return NextResponse.json({ question: await nextQuestion(input.provider, input.language, input.mode, input.topic, input.level, input.turns) });
    return NextResponse.json({ evaluation: await evaluate(input.provider, input.language, input.mode, input.turns) });
  } catch (error) {
