...
export async function POST(request: Request) {
  try {
    ...
    const provider = resolveTextProvider(input.provider);
    if (input.action === 'next') return NextResponse.json({ question: await nextQuestion(provider, input.language, input.mode, input.topic, input.level, input.turns, input.questionSetId) });
    return NextResponse.json({ evaluation: await transcriptTextEvaluator.evaluate(provider, input.language, input.mode, input.turns, input.uiLanguage) });
  } catch (error) {
