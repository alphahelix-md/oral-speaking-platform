...
const uiCopy: Record<UiLanguage, Record<string, string>> = {
  'zh-CN': { studio: '口语工作室', settings: '界面设置', interfaceLanguage: '界面语言', appearance: '显示模式', light: '浅色模式', dark: '深色模式', close: '关闭', confirm: '确定', daily: '你的每日口语空间', voice: '找到你的声音。', further: '继续前行。', intro: '真实对话。贴心反馈。看得见的进步。', begin: '开始练习', choose: '选择学习语言', journey: '你的历程', sessions: '次练习', minutes: '分钟口语', streak: '连续练习*', home: '首页', practice: '练习', progress: '进度', profile: '我的', settingsHint: '设置界面语言和显示模式。' },
...
export function OralApp() {
  ...
  const [uiLanguage, setUiLanguage] = useState<UiLanguage>('en'); const [theme, setTheme] = useState<Theme>('light'); const [settingsOpen, setSettingsOpen] = useState(false); const [draftUiLanguage, setDraftUiLanguage] = useState<UiLanguage>('en'); const [draftTheme, setDraftTheme] = useState<Theme>('light'); const [textProvider, setTextProvider] = useState<TextProvider>('openai'); const [accessCode, setAccessCode] = useState(''); const [draftTextProvider, setDraftTextProvider] = useState<TextProvider>('openai'); const [draftAccessCode, setDraftAccessCode] = useState('');
  ...
  async function finishSession(current: Session | null = session) { if (!current || busy && current === session) return; setBusy(true); setNotice(''); let evaluation = demoEvaluation(current.language, current.turns, uiLanguage); if (current.turns.length) { try { const response = await fetch('/api/ai', { method: 'POST', headers: { 'Content-Type': 'application/json', ...(accessCode ? { 'x-beta-access-code': accessCode } : {}) }, body: JSON.stringify({ action: 'evaluate', provider: textProvider, uiLanguage, language: current.language, mode: current.mode, level: current.level, topic: current.topic, turns: current.turns }) }); const data = await response.json(); if (response.ok && data.evaluation) evaluation = data.evaluation; else if (data.error !== 'AI_NOT_CONFIGURED') setNotice(extra.demoFeedback); } catch { setNotice(extra.demoOffline); } } const done = speakingReducer(current, { type: 'EVALUATE', evaluation }); setSession(done); saveSession(done); navigate('result'); setBusy(false); }
  function retry(turn: Turn) { if (!session) return; setSession(s => s && speakingReducer(s, { type: 'RETRY', turnId: turn.id })); setTranscript(''); setAudio(null); setSeconds(0); setNotice(extra.sameQuestion); setRetryExamPart(mode === 'ielts' ? turn.examPart || ieltsPartAt(topic, Math.max(0, session.turns.findIndex(item => item.id === turn.id))) : undefined); navigate('speaking'); }
  ...
  return <div className="app-shell"><div className="app-frame">
    ...
    {settingsOpen && <div className="settings-backdrop" role="presentation" onClick={closeSettings}>
      <section className="settings-sheet" role="dialog" aria-modal="true" aria-label={uiCopy[draftUiLanguage].settings} onClick={event => event.stopPropagation()}>
        ...
        <div className="settings-scroll">
          ...
          <div className="preference-module"><div className="module-title"><Sparkles size={18} /><span>{uiExtra[draftUiLanguage].provider}</span></div><div className="provider-options">{(['openai', 'deepseek', 'glm'] as const).map(provider => <button key={provider} className={draftTextProvider === provider ? 'selected' : ''} onClick={() => setDraftTextProvider(provider)}>{provider === 'glm' ? 'GLM' : provider === 'openai' ? 'OpenAI' : 'DeepSeek'}<Check size={16} /></button>)}</div><p className="provider-note">{uiExtra[draftUiLanguage].providerNote}</p><label className="access-code-label" htmlFor="beta-access-code">{uiExtra[draftUiLanguage].accessCode}</label><input id="beta-access-code" className="access-code-input" value={draftAccessCode} onChange={event => setDraftAccessCode(event.target.value)} autoComplete="off" /></div>
    ...
    <main className={`main-content ${page === 'speaking' || page === 'result' ? 'full-height' : ''}`}>
      {(page === 'progress' || page === 'profile') && <button className="text-back" onClick={goBack}><ArrowLeft size={18} /> {extra.back}</button>}
