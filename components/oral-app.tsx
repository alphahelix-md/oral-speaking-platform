...
export function OralApp() {
  ...
  function startSession() {
    ...
    setTranscript('');
    setAudio(null); setAudioMetrics(null); setTranscriptResult(null);
    setSeconds(0);
    setNotice('');
  ...
  async function beginRecording() {
    ...
    try {
      ...
      setAudio(null); setAudioMetrics(null); setTranscriptResult(null); setTranscript(''); setSeconds(0);
  ...
  async function analyzeAudio(blob: Blob) {
    ...
    try {
      ...
      for (const [index, chunk] of chunks.entries()) {
        ...
        if (data.text) parts.push(String(data.text).trim());
      }
      ...
      setNotice(reason === 'GLM_NOT_CONFIGURED' ? speechErrors[uiLanguage].glmMissing : reason === 'GLM_KEY_INVALID' ? speechErrors[uiLanguage].glmInvalid : reason === 'GLM_QUOTA_OR_LIMIT' ? speechErrors[uiLanguage].glmQuota : reason === 'EMPTY_TRANSCRIPT' ? speechErrors[uiLanguage].emptyTranscript : speechErrors[uiLanguage].transcriptionFailed);
    } finally {
  ...
  function retry(turn: Turn) { if (!session) return; setSession(s => s && speakingReducer(s, { type: 'RETRY', turnId: turn.id })); setTranscript(''); setAudio(null); setAudioMetrics(null); setTranscriptResult(null); setSeconds(0); setNotice(extra.sameQuestion); setRetryExamPart(mode === 'ielts' ? turn.examPart || ieltsPartAt(topic, Math.max(0, session.turns.findIndex(item => item.id === turn.id))) : undefined); navigate('speaking'); }
