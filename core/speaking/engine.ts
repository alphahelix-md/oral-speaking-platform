...
export type Action =
  ...
  | { type: 'ANSWER'; transcript: string; audioId?: string; durationSeconds: number; examPart?: 1 | 2 | 3 }
...
export function speakingReducer(session: Session, action: Action): Session {
  switch (action.type) {
    ...
    case 'ANSWER': {
      ...
      const turn: Turn = { id: crypto.randomUUID(), question: session.question, transcript: action.transcript, audioId: action.audioId, durationSeconds: action.durationSeconds, createdAt: new Date().toISOString(), attempt, examPart: action.examPart };
