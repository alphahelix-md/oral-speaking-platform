...
| Long answer | Record 1–3 minutes; interrupt after an early successful chunk | Successful chunks remain; pending/uncertain state explicit; manual completion available | 104s original replay confirmed on vivo X100s Pro / Chrome (872088f); user saw no text before or after refresh. Successful-chunk recovery not yet observed on device; recovery-notice fix awaits deployment |
...
| Recovery explanation | Resume a draft with an unknown transcription result, then play the question via cloud/local voice | Empty/partial recovery explanation stays visible; saved text/audio stay intact; no repeated STT | Production-handler regression tests pass; fix not yet deployed or phone-verified |
