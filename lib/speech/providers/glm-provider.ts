...
export class GlmSpeechProvider implements SpeechProvider {
  ...
  async transcribe(audio: File, language: LanguageId, requestId?: string): Promise<TranscriptResult> {
    ...
    console.info('[TRANSCRIBE_PROVIDER] request', { requestId, provider: this.id, model: process.env.GLM_TRANSCRIBE_MODEL || 'glm-asr-2512', language, fileSize: audio.size, mimeType: audio.type });
    ...
    console.info('[TRANSCRIBE_PROVIDER] success', { requestId, provider: this.id, status: response.status, textLength: String(data.text || '').length });
