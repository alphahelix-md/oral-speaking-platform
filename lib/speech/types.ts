...
export type PauseInterval = { startSeconds: number; endSeconds: number; durationSeconds: number };
export type AudioMetrics = { durationSeconds: number; speakingDurationSeconds: number; silenceDurationSeconds: number; silenceRatio: number; pauseCount: number; longPauseCount: number; longPauseIntervals: PauseInterval[]; averagePauseDurationSeconds: number; audioEnergy: number; volumeVariation: number; analysisAvailable: boolean };
