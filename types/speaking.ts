...
export type AnswerDraft = {
  ...
  examPart?: 1 | 2 | 3;
  stage: 'editing' | 'recording' | 'transcribing' | 'ready' | 'interrupted';
  chunks: TranscriptionChunk[];
};
export type EvaluationRun = { key: string; status: 'running' | 'succeeded' | 'degraded' };
export type Session = { revision?: number; draft?: AnswerDraft; evaluationRun?: EvaluationRun; id: string; language: LanguageId; mode: ModeId; level: string; topic: string; status: SessionStatus; question: string; turns: Turn[]; startedAt: string; endedAt?: string; evaluation?: Evaluation; examType?: string | null; examSetId?: string };
